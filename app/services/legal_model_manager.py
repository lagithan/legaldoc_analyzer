...
class LegalModelManager:
    def __init__(self):
        ...
        # Fast caching system for performance optimization
        self.analysis_cache = {}
        ...
        # Convert keywords to sets for O(1) lookup optimization
        self.legal_keyword_sets = self._convert_to_sets()
        ...
        # Deferred model loading for faster startup
        self.model_loaded = False
        self.model_loading_attempted = False
        
        # Load model on initialization (optimized for speed)
    ...
    def _compile_legal_patterns(self):
        """Pre-compile regex patterns for maximum speed optimization"""
    ...
    def _convert_to_sets(self) -> Dict[str, Set[str]]:
        """Convert keyword lists to sets for O(1) lookup optimization"""
        sets_dict = {}
        for category, keywords in self.legal_keywords.items():
            word_set = set()
            for phrase in keywords:
                word_set.update(phrase.lower().split())
            sets_dict[category] = word_set
        return sets_dict
    ...
    def load_fast_model(self):
        ...
        try:
            ...
            logger.info(f"✅ Fast Legal BERT Small model loaded successfully in {load_time:.2f}s!")
    ...
    def get_legal_embeddings(self, text: str) -> np.ndarray:
        """Get embeddings from the fast legal model with caching optimization"""
        ...
        try:
            ...
            embedding = embeddings[0]  # Return single embedding vector
    ...
    def calculate_keyword_strength_optimized(self, text: str, detected_terms: List[str]) -> float:
        ...
        # Quick check for common legal words using optimized sets
        common_legal = {'agreement', 'contract', 'terms', 'liability', 'payment'}
        text_words = set(re.findall(r'\b\w+\b', text.lower()[:500]))
        legal_count = len(common_legal.intersection(text_words))
    ...
    def detect_legal_terminology_fast(self, text: str, embeddings: np.ndarray = None) -> List[str]:
        ...
        try:
            ...
            # Fast keyword-based detection using optimized sets
            for concept, keyword_set in self.legal_keyword_sets.items():
                text_words = set(re.findall(r'\b\w+\b', text_lower[:1000]))  # First 1000 chars for speed
                if keyword_set.intersection(text_words):
                    detected_terms.append(concept)
            
            # Fallback to original method if sets didn't work
            ...
            if not detected_terms and embeddings is not None and self.legal_concept_embeddings:
    ...
    def detect_risk_patterns_fast(self, text: str) -> List[str]:
        ...
        try:
            ...
            if hasattr(self, 'compiled_patterns'):
                risk_mapping = {
                    'Unlimited Liability': self.compiled_patterns['liability_unlimited'],
                    'Indemnification Risk': self.compiled_patterns['indemnification'],
                    'Immediate Termination': self.compiled_patterns['termination_immediate'],
                    'Payment Issues': self.compiled_patterns['payment_terms'],
                    'General Liability': self.compiled_patterns['liability_general']
                }
                
                for risk_type, pattern in risk_mapping.items():
                    if pattern.search(text[:2000]):  # Search first 2000 chars for speed
    ...
    def identify_document_sections_fast(self, text: str) -> List[str]:
        ...
        try:
            ...
            # Use compiled patterns for speed if available
            if hasattr(self, 'compiled_patterns'):
                section_patterns = {
                    'Payment Terms': self.compiled_patterns['payment_terms'],
                    'Liability': self.compiled_patterns['liability_general'],
                    'Contract Terms': self.compiled_patterns['contract_terms'],
                    'Compliance': self.compiled_patterns['compliance']
                }
                
                for section_name, pattern in section_patterns.items():
                    if pattern.search(text[:2000]):  # Search first 2000 chars
    ...
    def analyze_legal_text_fast(self, text: str) -> Dict:
        ...
        if text_hash in self.analysis_cache:
            cached_result = self.analysis_cache[text_hash].copy()
            cached_result["analysis_metadata"]["cache_used"] = True
            cached_result["analysis_metadata"]["processing_time_ms"] = (time.time() - start_time) * 1000
            logger.info(f"⚡ Cache hit! Analysis completed in {cached_result['analysis_metadata']['processing_time_ms']:.1f}ms")
        ...
        try:
            ...
            # Cache the result for future use
            self.analysis_cache[text_hash] = analysis.copy()

            processing_time = (time.time() - start_time) * 1000
            analysis["analysis_metadata"]["processing_time_ms"] = processing_time
            
            logger.info(f"✅ Fast legal analysis complete in {processing_time:.1f}ms: "
                       f"Confidence={confidence_scores.get('overall_confidence', 0):.3f}, "
                       f"Complexity={analysis.get('legal_complexity', 0):.3f}")
    ...
    def analyze_legal_text(self, text: str, task: str = "fast") -> Dict:
        ...
        if task == "dual_analysis":
            ...
            base_analysis = self.analyze_legal_text_fast(text)
            
            # Simulate dual model response for compatibility
            return {
                "models_used": ["Legal BERT Small (Fast)", "Pattern Matcher"],
                ...
                "financial_terms": ["payment", "fee"] if any(t in text.lower() for t in ["payment", "fee"]) else [],
                ...
                "financial_terms_count": len([t for t in ["payment", "fee"] if t in text.lower()]),
                "temporal_terms_count": len([t for t in ["due", "term"] if t in text.lower()]),
